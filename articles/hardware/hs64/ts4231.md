...
format: `ts4231v1-uncalibrated_<filecount>.csv`. Because `CsvWriter` is a _sink_ operator, its
...
<xref:OpenEphys.Onix1.TS4231V1LinearTransform> which takes `TS4231V1PositionDataFrame` as its
...
[MemberSelector](xref:Bonsai.Expressions.MemberSelectorBuilder) operator on the `CsvWriter`
selects members from `TS4231V1PositionDataFrame`. The easiest way to place a `MemberSelector` is
to right-click the `CsvWriter` node, hover over the "Output" option, and then click the
relevant members that appear in the context menu. The
